...
check_is_readcount_file = function(infolder){
    ...
    random_choosen_file = paste0(infolder,'/',files[sample(1,length(files),1)])

    random_choosen_file_concept = read.table(random_choosen_file, header=F, sep='\t', stringsAsFactors=F)
...
compute_DEG = function(current_dir, infolder, species, output){
    files = list.files(infolder)
    treat_files = paste0(infolder,'/',files)
    ...
    if (length(treat_files) != 0 & length(control_files) != 0){
        ...
        if (check_is_readcount_file(control_files) == 'True'){
            ...
            control_counts = txrsem$counts
        }
        # load treat files
        if (check_is_readcount_file(infolder) == 'TRUE'){
            ...
            treat_counts = treat_txrsem$counts
        }
        ...
        total_counts = total_counts[,-c('gene_id')]
        ...
        dds = DESeqDataSetFromMatrix(countData = total_counts, colData = colData, condition = ~ condition)
        ...
        res_mat = res_mat[order(-res_mat$fc, res_mat$pvalue),]
